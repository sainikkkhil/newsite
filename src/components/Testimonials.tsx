...
const Testimonials = () => {
  ...
  return <section className="w-full py-20 px-6 md:px-12 bg-card relative overflow-hidden">
      {/* Background grid */}
      <div className="absolute inset-0 cosmic-grid opacity-20"></div>
      
      <div className="max-w-7xl mx-auto space-y-16 relative z-10">
        <div className="text-center space-y-4 max-w-3xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-medium tracking-tighter text-foreground">Trusted by innovative teams worldwide</h2>
          <p className="text-muted-foreground text-lg py-[15px] my-[3px] mx-[6px] px-[9px]">See how our platform transforms operations for businesses</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => <div key={index} className="p-6 rounded-xl border border-border bg-background/80 backdrop-blur-sm hover:border-border/60 transition-all duration-300">
              <div className="mb-6">
                {[...Array(5)].map((_, i) => <span key={i} className="text-primary inline-block mr-1">★</span>)}
              </div>
